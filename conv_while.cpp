...
void conv_while (hls::stream<uint_8_side_channel> &input,hls::stream<uint_8_side_channel> &output,char kernel[KERNEL_DIM*KERNEL_DIM][MAX_CHANNELS],int input_channels)//)//[MAX_CHANNELS],int choice)
{
//#pragma HLS INTERFACE ap_ctrl_none port=return

#pragma HLS INTERFACE axis port=input
#pragma HLS INTERFACE axis port=output
	//Testing multiple pragmas for kernel storage
...
#pragma HLS INTERFACE s_axilite port=input_channels bundle= CHANNELS
...
//#pragma HLS ARRAY_RESHAPE variable=line_buffer complete
		 ...
		 //{
			 ...
			 for (int idx_win_row=0; idx_win_row<KERNEL_DIM; idx_win_row++)
			 {
				 for (int idx_win_col=0; idx_win_col<KERNEL_DIM; idx_win_col++)
				 {
					 ...
					 val=(short)kernel[idx_win_row*KERNEL_DIM+idx_win_col][channel]*val;//[channel];
